...
const main = async () => {
  ...
  const ContractsHandlerFactory = await hre.ethers.getContractFactory("ContractsHandler");
  ...
  const HandlerContract = await ContractsHandlerFactory.deploy();
  console.log("Contracts Handler ADDRESS ----", HandlerContract.address);
